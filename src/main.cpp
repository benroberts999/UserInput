...
int main() {
  ...
  // InputBlock ib("name1", {{"key1", "1"}, {"key2", "3.7"}, {"key3",
  // "hello"}});

  InputBlock ib("name1", std::ifstream{"input.in"});

  // ib.add({{"key1", "val1"}, {"key2", "val2"}});
  ib.add(InputBlock("blockA", {{"keyA", "valA"}, {"keyA2", "valA2"}}));
  ib.add(InputBlock("blockB", {{"keyB1", "valB"}, {"keyB2", "valB2"}}));
  ib.add(InputBlock("blockB", {{"keyB2", "17.3"}, {"keyB3", "16.3"}}));
  ...
  std::cout << ib.getBlock("blockB")->get("keyB2", 15.1) << " / " << 17.3
            << "\n";
  ...
  */
  ...
  std::string block_string =
      ...
      "   keyB2 = 17.3;    keyB3 = 16.3;  }  blockC { keyC = valC; blockD { "
      "keyC "
